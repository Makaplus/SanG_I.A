...
def main():
    ...
    for f in files:
        ...
        ok_rev, msg_rev = run_worker(Path("scripts/workers/revisione.py"), ["--file", str(f), "--pages", "4", "--dpi", "320"])
        if not ok_rev:
            auto_fail += 1
            continue
        ...
        if (tipo2 or "").upper() == "OCC" and has_r and has_a and has_p:
            ok_rn, _ = run_worker(Path("scripts/workers/organizzatore_rename.py"), ["--file", str(f)])
            ...
            ok_mv, _ = run_worker(Path("scripts/workers/organizzatore_percorso.py"), ["--file", path_for_move])
            if ok_rn and ok_mv:
                auto_ok += 1
            else:
                auto_fail += 1
