...
def main():
    ...
    for f in files:
        ...
        if ext in (".doc", ".docx", ".rtf", ".txt"):
            ok_doc, msg_doc = run_worker(Path("scripts/workers/organizzatore_doc_reader.py"), ["--file", db_nome_file])
        ...
        # ==========================
        if tipo_up != "OCC":
            _ok_probe, _msg_probe = run_worker(Path("scripts/workers/organizzatore_altro_probe.py"), ["--file", db_nome_file])
            ...
            if filename_has_occ_token(db_nome_file):
                _ok_rn_fix, _msg_rn_fix = run_worker(Path("scripts/workers/organizzatore_rename.py"), ["--file", db_nome_file])
            ...
            if anno:
                ok_mv, msg_mv = run_worker(Path("scripts/workers/organizzatore_percorso.py"), ["--file", db_nome_file])
                mv_state = mark(ok_mv)
            ...
            ok_vv, msg_vv = run_worker(Path("scripts/workers/organizzatore_vari_verbali.py"), ["--file", db_nome_file])
            mv_state = mark(ok_vv)
        ...
        # ==========================
        if ext == ".pdf":
            ok, msg = run_worker(Path("scripts/workers/organizzatore_occ_ocr_rgnr.py"), ["--file", db_nome_file])
            if ocr_state == "⏭":
                ocr_state = mark(ok)
        ...
        if has_rgnr and has_anno and has_proc and (row2[9] or "").upper() not in ("NEEDS_READER", "NEEDS_DEEPER_ANALYSIS", "STANDBY"):
            ok_rn, msg_rn = run_worker(Path("scripts/workers/organizzatore_rename.py"), ["--file", row2[1]])
            rn_state = mark(ok_rn)
            if not ok_rn and not note_msg:
                note_msg = msg_rn

            row3 = fetch_doc_by_sha1(sha1)
            ok_mv, msg_mv = run_worker(Path("scripts/workers/organizzatore_percorso.py"), ["--file", row3[1]])
            mv_state = mark(ok_mv)
            if not ok_mv and not note_msg:
                note_msg = msg_mv
    ...
    # report
    ok_rep, msg_rep = run_worker(Path("scripts/workers/organizzatore_report_errori.py"), [])
    ...
    # ROUTER REVISIONE
    ok_router, msg_router = run_worker(Path("scripts/workers/organizzatore_revisione_router.py"), [])
