...
- `scripts/workers/organizzatore_occ_ocr_rgnr.py`: estrazione OCC specifica.
- `scripts/workers/organizzatore_rename.py`: naming canonico OCC e label-fix NON-OCC.
- `scripts/workers/organizzatore_percorso.py`: move in cartelle anno.
- `scripts/workers/organizzatore_vari_verbali.py`: fallback NON-OCC.
- `scripts/workers/organizzatore_revisione_router.py`: retry/reinstradamento revisione.
- `scripts/01_reader.py`, `02_preparatore.py`, `03_embeder.py`: pipeline dati/testo/embedding.
...
- Mantieni **un solo entrypoint ufficiale** (`start.bat` su Windows, `scripts/start.sh` su Linux).
...
- Estrai in `scripts/workers/common_patterns.py`:
  - regex RGNR,
...
```bash
# installazione ambiente da zero (Windows)
install_sangia.bat
# oppure
powershell -ExecutionPolicy Bypass -File .\install_sangia.ps1
...
- Se `start.bat` non parte, esegui prima `install_sangia.bat` per ricreare/aggiornare ambiente.
- Se compare `SyntaxError` su `organizzatore.py`, esegui:
  ```bash
  venv\Scripts\python.exe -m py_compile organizzatore.py webapp\app.py
  ```
  e ripristina eventuali file corrotti dal tuo branch/repo.
